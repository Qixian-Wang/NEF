...
if __name__ == '__main__':
    ...
    def process_data():
        for process_index in range(size):
            if process_index % size == rank:
                print(f'rank {rank} activated')
                for idx, zip_arg in enumerate(itertools.islice(zip(data.load()), rank, 2, size)):
                    result = bandpass_filter(*zip_arg)
                    sequence_index = rank + size * idx
                    print(f'segment {sequence_index} is filtered by rank {rank}')
                    yield sequence_index, result
    ...
    comm.Barrier()
    for sequence_index, res in process_data():
        print(res.shape)
