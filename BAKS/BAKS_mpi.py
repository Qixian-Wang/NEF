...
from miv.core.operator.policy import StrictMPIRunner, SupportMPIMerge
...
if __name__ == '__main__':
    ...
    if rank == 0:
        ...
        data_full = None  # Initialize `data_full` as None for non-root ranks

    # Broadcast number of channels and samples
    num_channels = comm.bcast(data_full.number_of_channels if rank == 0 else None, root=0)
    num_samples = comm.bcast(data_full.data.shape[0] if rank == 0 else None, root=0)

    # Initialize the array for non-root ranks
    if rank != 0:
        data_full_data = np.empty((num_samples, num_channels))
    else:
        data_full_data = data_full.data.copy()

    # Broadcast column by column
    for i in range(num_channels):
        column_data = comm.bcast(data_full.data[:, i] if rank == 0 else None, root=0)
        data_full_data[:, i] = column_data

    # Broadcast timestamps
    data_full_timestamps = comm.bcast(data_full.timestamps if rank == 0 else None, root=0)

    # After broadcast, reconstruct `data_full` in all ranks
    data_full = Signal(data=data_full_data, timestamps=data_full_timestamps,
                       rate=comm.bcast(data_full.rate if rank == 0 else None, root=0))

    print(f"Hello from rank {rank} of {size} total ranks.")
    comm.Barrier()
    start_time = MPI.Wtime()

    for channel in range(rank, data_full.number_of_channels, size):
        data_single_channel = Signal(
            data=np.expand_dims(data_full.data[:, channel], axis=1),
            timestamps=data_full.timestamps,
            rate=data_full.rate
        )
        filtered_data = bandpass_filter(data_single_channel)
        print(f"channel {channel} is handled by rank {rank}")
    ...
    with open(summary_file_path, 'w') as summary_file:
        summary_file.write(f"time: {execution_time}\n")