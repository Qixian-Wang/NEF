...
def _bayesian_adaptive_kernel_smoother(spike_times, time, a, b):
    ...
    local_numerator = 0.0
    local_denumerator = 0.0

    for i in range(rank, spike_number_total, size):
        val = (((time - spike_times[i]) ** 2) / 2 + 1 / b)
        local_numerator += val ** -a
        local_denumerator += val ** -(a + 0.5)
        print(f'number {i} finished by {rank}')

    numerator = comm.allreduce(local_numerator, op=MPI.SUM)
    denumerator = comm.allreduce(local_denumerator, op=MPI.SUM)
